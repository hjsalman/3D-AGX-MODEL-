...
ant_dx = 9.5;  ant_dz = 21.6;   // antenna center offset from mounting-hole center (TI IWR Gerber): 9.5mm sideways, 21.6mm up
...
imu_cx=-60; imu_screw=2.4;     // M2 clearance; IMU (Phidgets MOT0110) mounts INSIDE the box, left of the mmWave (free side)
...
// ---- width ----
half = max(zed_w/2, ant_dx + rad_w/2) + edge; IW = 2*half; OW = IW + 2*wall;
// ---- heights ----
...
rad_cx=ant_dx;   // shift mounting holes so the REAL antenna (up-left in Reza photo) lands on X=0 (ZED center)
...
module imu_mount(){   // 4 screw holes through front wall for the external IMU (Phidgets 1205)
