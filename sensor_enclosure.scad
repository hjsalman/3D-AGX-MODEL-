...
jack_gap=42; rad_stick=3; rad_standoff=15;             // IWR base just barely crosses the front panel plane (min protrusion, per Sur); DCA on 15mm standoffs
...
module imu_mount(){   // 4x M2 through the FRONT wall; IMU mounts inside, left of the mmWave
    translate([imu_cx,-eps,rad_cz]) rotate([-90,0,0]) holes4(imu_hole_dx,imu_hole_dy) cylinder(d=1.7,h=wall+2*eps);
