...
ant_dx = 34;  ant_dz = 8;  ant_x = 0;   // antenna offset from the 4-screw center, measured off Reza marked photo (34mm to the side, 8mm up). ant_x = extra shift right of ZED center (0 = antenna on center)
...
jack_gap=42; rad_stick=25; rad_standoff=15;            // IWR protrudes 25 past the panel (clear of beam); DCA on 15mm standoffs (per reference box)
...
// ---- IMU (Phidgets 1205, 4 screw holes) mounted external, on the free side of the mmWave opening ----
imu_cx=-60; imu_screw=2.4;   // M2 clearance; IMU (Phidgets MOT0110) mounts INSIDE, left of the mmWave (front wall)
imu_hole_dx=22.86; imu_hole_dy=27.94;   // VERIFIED from Phidgets MOT0110 mechanical drawing (0.900 x 1.100 in), M2 bolts, enclosure 37.8 x 39.0
...
rad_cx=ant_dx+ant_x;   // mounting holes shifted so the antenna lands at X = ant_x (right of the ZED centerline)

hm_dz=46; handle_reach=95; hsec=18; grip_rise=52; grip_back=190;   // longer reach (95, ~77mm hand clearance) + longer grab bar (190), grip behind AGX
handle_y=agx_cy-10; handle_z=OH*0.52;   // foot over the AGX; grab bar extends back behind the AGX

// =====================================================================
module rbox(w,d,h,r=4){ hull() for(x=[r,w-r],y=[r,d-r]) translate([x,y,0]) cylinder(r=r,h=h); }

// --- ISO metric thread cutter: subtract to tap a hole of nominal dia D (M3 P=0.5, M2 P=0.4) ---
module mthread(D,P,L,fn=20){
    Rmin=D/2-0.6134*P;
    linear_extrude(height=L,twist=-360*L/P,convexity=8,$fn=fn)
      union(){ circle(r=Rmin,$fn=fn);
        translate([Rmin,0]) polygon([[0,-P/2],[0.6134*P,-P/8],[0.6134*P,P/8],[0,P/2]]); }
}
module tap3(L){ translate([0,0,-eps]) mthread(3,0.5,L+2*eps); }   // M3 tapped
module tap2(L){ translate([0,0,-eps]) mthread(2,0.4,L+2*eps); }   // M2 tapped
...
module rad_window(){ translate([rad_cx,-eps,rad_cz]) ycut(rad_w+clr, rad_h+clr, wall+2*eps, 10); } // opening in the front wall; IWR protrudes 25mm through/past it
module rad_standoffs(){   // 4x 15mm standoffs raising the DCA off the mount plate (reference dimension)
    translate([rad_cx, rad_back, rad_cz]) rotate([-90,0,0]) holes4(rad_hole_dx, rad_hole_dy)
        difference(){ cylinder(d=8,h=rad_standoff); tap3(rad_standoff); }
}
module rad_plate(){
    fh=rad_hole_dy+22; py=rad_back+rad_standoff;
    difference(){
...
module imu_mount(){   // 4x M2 through the FRONT wall; IMU mounts inside, left of the mmWave
    translate([imu_cx,-eps,rad_cz]) rotate([-90,0,0]) holes4(imu_hole_dx,imu_hole_dy) mthread(2,0.4,wall+2*eps);
...
module agx_mount(){ translate([0,agx_cy,floor_th]) holes4(agx_mt,agx_mt) difference(){
        cylinder(d=boss_od,h=agx_so); translate([0,0,-1]) mthread(3,0.5,agx_so+3); } }
...
module box_body(){
    union(){
        difference(){
            shell(); cavity();
            rad_window(); imu_mount(); zed_window(); rear_cables(); agx_screws(); handle_mounts();
        }
        agx_mount(); handle_nuts(); rad_plate(); rad_standoffs(); zed_plate();
    }
}

module handle(){   // foot on side wall -> reach arm out -> grab bar runs REARWARD, grip behind the AGX
    ...
    translate([ft-1,-hsec/2,-hsec/2]) cube([handle_reach-ft+1, hsec, hsec]);                 // reach arm out
    translate([handle_reach-hsec,-hsec/2,-hsec/2]) cube([hsec, grip_back+hsec/2, hsec]);         // grab bar, runs to the back
    translate([handle_reach-hsec, grip_back-hsec/2,-hsec/2]) cube([hsec, hsec, grip_rise+hsec/2]); // end stop up at the rear
}
...
module gc(){
    ...
    color([0.9,0.85,0.4]) translate([-ant_win/2,-rad_stick-1.5,rad_cz+ant_dz-ant_win/2]) cube([ant_win,2,ant_win]);   // antenna: X=0, up by ant_dz
    color([0.15,0.6,0.3]) translate([imu_cx-37.8/2, wall, rad_cz-39/2]) cube([37.8, 13.4, 39]);   // IMU ghost (Phidgets MOT0110 case) INSIDE, against front wall
}
module assembly(){
    box_body();
...
// render select
part="assembly";
if(part=="assembly")assembly(); else if(part=="box_body")box_body(); else if(part=="handles")handles(); else if(part=="handle")handle();
