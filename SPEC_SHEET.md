...
## MATERIAL / METAL AMOUNT
- Build: 3003 aluminum (6063-T3 acceptable), 2.0 mm (0.080") thick, bent into 2-3 panels, screwed together.
- AMOUNT TO BUY: half a 4x4 ft sheet = 24 x 48 in (610 x 1220 mm). One piece.
  Reasoning: the panels (front, back, 2 sides, bottom; open top) flatten to ~0.24 m2. This sheet is
  ~0.74 m2, so ~3x the area needed - covers the bend flanges, the overlap tabs, and a couple of mistakes.
- Open top (no lid/glass). All mounts are through-holes (screw + nut).

## FASTENER SCHEDULE

| Mount | Screw | Qty | Hole pattern (mm) | Hole in printed part | Washer / seat | Length (approx) |
|---|---|---|---|---|---|---|
| mmWave DCA1000 | M3 | 4 | 76.30 x 56.89 | through-hole + nut, 4x M3x15 standoffs | washer both ends | M3 x 22 |
| ZED 2i | M3 | 4 | 153.4 x 14 | through-hole into ZED threaded holes | washer | M3 x 8 |
| Jetson AGX | M3 | 4 | 97 x 97 | through-hole + nut, 4x M3x6 standoffs | washers | M3 x 12 |
| IMU (Phidgets 1205) | M2 | 4 | 22.86 x 27.94 | through-hole + nut | washers | M2 x 8 |
| Handles (each) | M3 | 2 | 46 apart | through-hole + nut | washers | M3 x 16 |

Standoffs: mmWave 4x M3x15, AGX 4x M3x6 (spacers). Panel joints: ~16x M3x8 + nuts (or 1/8in Al rivets).

Hole / seat reference dims used in the model:
- M3 clearance hole Ø3.4 ; M2 clearance hole Ø2.4
- Washer seat Ø8 x 2.0 ; counterbore Ø9 x 3.0 ; small counterbore Ø7 x 2.5
- M3 hex nut pocket 6.6 mm across-flats x 2.6 mm ; heat-set boss Ø8, insert Ø4.2 x 6
- Threaded holes are self-tapping pilot holes: M3 -> Ø2.6, M2 -> Ø1.7 (screw thread-forms into the print). Heat-set inserts (M3 Ø4.2) optional for reusable metal threads.

Tapped (screw threads into the print): mmWave standoffs, AGX bosses, IMU wall.
Clearance + nut: ZED plate, handle mounts (captive nut).
